...
function generateReactEmblemComponent() {
  function spokes(cx, cy, rIn, rOut, count) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      res.push(`<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="currentColor" strokeWidth="0.8" />`);
    }
    return res.join('\n        ');
  }
...
`;
  ...
  fs.writeFileSync(path.join(__dirname, 'proposed_StateEmblem.tsx'), code, 'utf8');
  console.log("Wrote proposed_StateEmblem.tsx successfully! Length:", code.length);
}

generateReactEmblemComponent();
